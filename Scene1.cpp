...
// === Showcase timeline ===
// Scene0 : hold 5s, fade to black 4s
// Scene1 : run animation, linger 5s after it completes, fade to black 4s
// Scene2 : hold indefinitely (no final fade)
static const DWORD K_SCENE0_FADE_IN_DELAY_MS    = 2500u;
static const DWORD K_SCENE0_FADE_IN_MS          = 1000u;
static const DWORD K_SCENE0_HOLD_MS             = 5000u;
static const DWORD K_SCENE0_FADE_TO_BLACK_MS    = 4000u;
static const DWORD K_SCENE1_FADE_TO_BLACK_MS    = 4000u;
static const DWORD K_SCENE1_POST_ANIM_EXTRA_MS  = 5000u;
static const TCHAR K_SEQUENCE_AUDIO_PATH[]               = TEXT("Omega.wav");
...
static void StartShowcaseAudio(void)
{
    ...
    if (PlaySound(K_SEQUENCE_AUDIO_PATH, NULL, SND_FILENAME | SND_ASYNC | SND_NODEFAULT | SND_LOOP))
    {
...
static void EnterSequenceState(SequenceState state)
{
    ...
    case SEQUENCE_SCENE0_HOLD:
        ...
        UpdateBlendFadeInternal(gCtx_Switcher.gFade);
        break;
    case SEQUENCE_SCENE0_FADE_TO_BLACK:
        gActiveScene = ACTIVE_SCENE_SCENE0; // fade current scene to black
        break;
    case SEQUENCE_SCENE1_HOLD:
        gActiveScene = ACTIVE_SCENE_SCENE1;
        gCtx_Switcher.gFade = 1.0f; // cut in fully visible
...
static void UpdateShowcaseSequenceInternal(void)
{
    ...
    case SEQUENCE_SCENE0_HOLD:
        ...
        gCtx_Switcher.gFade = 1.0f; // fully visible
        UpdateBlendFadeInternal(gCtx_Switcher.gFade);
        if (elapsed >= K_SCENE0_HOLD_MS) EnterSequenceState(SEQUENCE_SCENE0_FADE_TO_BLACK);
        break;

    case SEQUENCE_SCENE0_FADE_TO_BLACK:
        fadeProgress = Clamp01((now - sSequenceStateStartMs) / (float)K_SCENE0_FADE_TO_BLACK_MS);
        gCtx_Switcher.gFade = 1.0f - fadeProgress; // fade to black
        UpdateBlendFadeInternal(gCtx_Switcher.gFade);
        if (fadeProgress >= 1.0f) EnterSequenceState(SEQUENCE_SCENE1_HOLD);
        break;
